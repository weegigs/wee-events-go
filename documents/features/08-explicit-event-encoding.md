...
constructor takes an explicit `we.Encoder`, a publish site may override it explicitly
via `we.WithEncoder`, and there is no code path that selects an encoding the caller did
not name. JSON remains the *recommended* interop encoding —
...
and override it explicitly at a publish site when a payload warrants it, so that every
encoding in use is a reviewable line of code.*
...
- **ENCODING-S2.R3** (ubiquitous) — `PublishOptions` shall support an explicit
  per-publish override via `we.WithEncoder(encoder Encoder)`; when present it selects
  the encoder for that publish only, taking precedence over the store's constructor
  encoder. *(Both scopes are caller-stated — an override is explicit, a default nobody
  wrote is not. Mixed-encoding streams are already a supported read scenario:
  CBOR-S2.R2.)*
- **ENCODING-S2.R5** (unwanted) — If `WithEncoder` is supplied with a `nil` encoder,
  then `Publish` shall return an error of the form `"<store>: encoder must not be nil"`
  and record nothing — never silently fall back to the constructor encoder. *(Failure
  companion to R3: an explicit-but-broken override is an error, not a fallback.)*
